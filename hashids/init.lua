...
if ok then
	...
	str_switch_pos = function(str, pos1, pos2)
		...
		local a,b = substr(str,pos1,pos1), substr(str, pos2, pos2);
		if pos1 > pos2 then return gsub(gsub(str, a, b, 1), b, a, 1); end
		return gsub(gsub(str, b, a, 1), a, b, 1);
	end
end
...
local function hash(number, alphabet)
	...
	repeat
		push(buffer, 1, gcap(alphabet, (number % alen)))
...
local function unhash(input, alphabet)
	local number, ilen, alen = 0, #input, #alphabet;

	for i=0, ilen do
		local cpos = (find(alphabet, gcap(input, i), 1, true) - 1);
...
local function consistent_shuffle(alphabet, salt)
	...
	local v, p = 0, 0;
	for i = (#alphabet - 1), 1, -1 do
		v = (v % slen);
		local ord = byte(salt, v+1)
		p = p + ord;
		local j = (ord + v + p) % i;
...
function hash_mt:encode(...)
	...
	for i, number in ipairs(numbers) do
		assert(type(number) == 'number', "all parameters must be numbers");
	...
	for i=1, #numbers do
		...
		if i < numbers_size then
			number = number % (byte(last) + (i - 1));
			ret = ret .. gcap(self.seps, (number % #self.seps));
		end
	end

	local guards_len = #self.guards;
	if #ret < self.min_hash_length then
		local guard_index = (hash_int + byte(ret, 1)) % guards_len;
		ret = gcap(self.guards, guard_index) .. ret;

		if #ret < self.min_hash_length then
			guard_index = (hash_int + byte(ret, 3)) % guards_len;
			ret = ret .. gcap(self.guards, guard_index);
		end
	end

	local half_len, excess = floor(alpha_len * 0.5), 0; -- alpha_len / 2
	while #ret < self.min_hash_length do
		alpha = consistent_shuffle(alpha, alpha);
		ret = strsub(alpha, half_len + 1) .. ret .. strsub(alpha, 1, half_len);

		excess = (#ret - self.min_hash_length);
		if excess > 0 then
			excess = (excess * 0.5);
			ret = strsub(ret, excess + 1, (excess + self.min_hash_length));
		end
	end
...
function hash_mt:decode(hash)
	...
	local parts, index = {}, 1;
	for part in gmatch(hash, "[^".. self.guards .."]+") do
		parts[index] = part;
		index = index + 1;
	end
	...
	t = substr(t, 2); -- then put the rest in t
	
	parts, index = {}, 1;
	for part in gmatch(t, "[^".. self.seps .."]+") do
		parts[index] = part;
		index = index + 1;
	end
